...
class VoiceDataset(Dataset):
    def __init__(self, common_voice_dataset, num_examples=None, tokenizer=None):
        self.dataset = common_voice_dataset
        self.num_examples = (
            min(num_examples, len(common_voice_dataset)) if num_examples is not None
...
def get_dataset(batch_size=32, num_examples=None, num_workers=4):
    dataset = load_dataset('MLCommons/peoples_speech','test' ,split="test")
    tokenizer = get_tokenizer()
    dataset = VoiceDataset(dataset, tokenizer=tokenizer, num_examples=num_examples)
    dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=True, collate_fn=collate_fn, num_workers=num_workers)
    return dataloader

if __name__ == "__main__":
    data = get_dataset(batch_size=32)
    for batch in data:
        audio=batch["audio"]
